...
class oos_datetime {
		...
		*/
		function date2stamp($date,$time = false){
			...
			//	commented out as untestable, strix 2012-05-10
		...
		*/
		function get_regex_date(){
			...
			foreach($this->date_order as $item)
			{
				...
				{
					case "d":		$regex .= "[0-9]{2}";						break;
					case "D":		$regex .= "[A-Z]{1}[a-z]{2}";				break;
					case "l":		$regex .= "[A-Z]{1}[a-z]{2,5}day";			break;
					case "j":		$regex .= "[1-9]{1}|[1-3]{1}[0-9]{1}";		break;
					case "n":		$regex .= "[1-9]{1}|1[0-2]{1}";				break;
					case "F":		$regex .= "[A-Z]{1}[a-z]{2,8}";				break;
					case "m":		$regex .= "[0-9]{2}";						break;
					case "M":		$regex .= "[A-Z]{1}[a-z]{2}";				break;
					case "y":		$regex .= "[0-9]{2}";						break;
					case "Y":		$regex .= "[0-9]{4}";						break;
				}
		...
		function get_regex_time(){
			...
			foreach($this->time_order as $item){
				...
				switch($this->time_map[$item]){
					case "h":		$regex .= "[0-1]{1}[0-9]{1}";			break;
					case "g":		$regex .= "[0-9]{1,2}";					break;
					case "H":		$regex .= "[0-9]{2}";					break;
					case "G":		$regex .= "[0-9]{1,2}";					break;

					case "i":
					case "s":		$regex .= "[0-9]{2}";					break;
				}
		...
		*/
		...
		function get_format_date()
		{
			...
			foreach($this->date_order as $item)
			{
				...
				switch($this->date_map[$item])
				{
					case "d":	$format .= "01 - 31";				break;
					case "D":	$format .= "Mon - Sun";				break;
					case "l":	$format .= "Monday - Sunday";		break;
					case "j":	$format .= "1 - 31";				break;
					case "n":	$format .= "1-12";					break;
					case "F":	$format .= "January - December";	break;
					case "m":	$format .= "01 - 12";				break;
					case "M":	$format .= "Jan - Dec";				break;
					case "y":	$format .= "YY";					break;
		...
		*/
		...
		function get_format_time(){
			foreach($this->time_order as $item)
			{
				...
				switch($this->time_map[$item])
				{
					case "h":	$format .= "01 - 12";			break;
					case "g":	$format .= "1 -12";				break;
					case "H":	$format .= "00 -23";			break;
					case "G":	$format .= "0 -23";				break;

					case "i":
					case "s":	$format .= "00 - 59";			break;
				}
				if($counter != sizeof($this->time_full))
				{
					switch($this->time_sep)
		...
		*/
		...
		function get_input_date($value = array())
		{
			foreach($this->date_order as $item)
			{
				switch($this->date_map[$item])
				{
					case "d":	$input .= "<input type='text' name='day' value='".$value['day']."' size='2'>";				break;
					case "D":	$input .= "<input type='text' name='day' value='".$value['day']."' size='3'>";				break;
					case "l":	$input .= "<input type='text' name='day' value='".$value['day']."' size='8'>";				break;
					case "j":	$input .= "<input type='text' name='day' value='".$value['day']."' size='2'>";				break;
					case "n":	$input .= "<input type='text' name='month' value='".$value['month']."' size='2'>";			break;
					case "F":	$input .= "<input type='text' name='month' value='".$value['month']."' size='12'>";			break;
					case "m":	$input .= "<input type='text' name='month' value='".$value['month']."' size='2'>";			break;
					case "M":	$input .= "<input type='text' name='month' value='".$value['month']."' size='3'>";			break;
					case "y":	$input .= "<input type='text' name='year' value='".$value['year']."' size='2'>";			break;
					case "Y":	$input .= "<input type='text' name='year' value='".$value['year']."' size='4'>";			break;
				}
			}
			return $input;
		}
