 ...
 *	Created: 7 July 2005
 *
 *	(TODO) there is a lot of code duplication in this object which could be trivially removed
 *	(TODO) validate arguments to mutator methods and make properties private
 */
...
class BARCODE
{
	...
	/**	
	 *	Height of barcode image, pixels
	 *	@var int
	 */
	...
	/**
	 *	Foreground color of rendered barcode, array {0 => red, 1 => green, 2 => blue}
	 *	@var int[int]
	 */
	...
	/**
	 *	Background color of rendered barcode, array {0 => red, 1 => green, 2 => blue}
	 *	@var int[int]
	 */
	...
	/**
	 *	Image format (CHECKME) list of allowed types?
	 *	@var string
	 */
	...
	/**
	 *	Constructor
	 *	@param	string	$encoding	(DOCUMENTME) set of allowed values
	 *	@return	void
	 */
	...
	function __construct($encoding = "EAN-13")
	{
		...
		$this->_bgcolor = array(255, 255, 255);
	...
	/**
	 *	Set .ttf font file to be used
	 *	@param	string	$font			Font file
	 *	@param	bool	$autolocate		Try to find a named font in windows font cache
	 *	@return	void	
	 */
	...
	/**
	 *	Set encoding scheme for generated barcode images
	 *
	 *	(TODO) check that encoding scheme is supported
	 *
	 *	@param	string	$encoding	(DOCUMENTME) list of allowed symbologies
	 *	@return	void
	 */
	...
	/**
	 *	Set foreground and background colors from hex string (no #)
	 *
	 *	@param	string	$color	Hex color string, eg FF00FF
	 *	@param	string	$color	Hex color string, eg 555555
	 *	@return	void
	 */
	 ...
	 *
	 *	(TODO) get list of allowable types and check $format against them
	 *
	 *	@param	string	$format	(DOCUMENTME) 'png' or...
	 *	@return	void
	 */
	...
	/**
	 *	(DOCUMENTME)
	 *	@param	int		$height 	Pixels? (CHECKME)
	 *	@return	void
	 */
	...
	/**
	 *	Set n2w - Narrow to Wide
	 *
	 *	@param	float	$n2w	Aspect ratio? (CHECKME)
	 *	@return	void
	 */
	...
	/**
	 *	Get the last error, or output to browser as an image file
	 *
	 *	@param	bool	$asimg		Output as a .png image
	 *	@return	string
	 */
	...
	function error($asimg = false)
	{
		...
		if ($asimg) {
			...
			@imagettftext(
				$im, 10, 0, 5, 50, $color, 
				$this->_font, wordwrap($this->_error, 40, "\n")
			);
			@imagepng($im);
			@imagedestroy($im);
		}
	 ...
	 *	(TODO) use existing format if none specified
	 *	
	 *	@param	string	$barnumber		Number / data to encode
	 *	@param	string	$format			(DOCUMENTME) 'gif' or ...
	 */
	...
	function genBarCode($barnumber, $format = 'gif', $file = '')
	{
		...
		switch(strtoupper($this->_encode)) {
			...
			case "UPC-A":
				...
				{
					$this->_error = "Barcode number must be less than 12 characters.";
					return false;
				}
				$this->_eanBarcode($barnumber, $this->_scale, $file);
				break;		//......................................................................
			...
			case "ISBN":
				...
				{
					$this->_error = "Barcode number must be 12 or 13 characters.";
					return false;
				}

				if (substr($barnumber,0,3) != "978")
				{
			...
			case "EAN-8":
				...
				{
					$this->_error = "Barcode number must be 8 characters or fewer.";
					return false;
				}
		...
		}	// end switch($this->_encoding)


		//	previous version of this, left in case of error with replacement, strix 2012-04-10
		/*
	 ...
	 *
	 *	@param	string	$barnumber	Data to be encoded
	 *	@return	string
	 */
	...
	function _c93Encode($barnumber)
	{
		...
		//	calculating C And K (DOCUMENTME) how is the checksum arrived at?
		...
		for ($j = 0; $j < 2; $j++)
		{
			...
			for ($i = strlen($barnumber); $i > 0; $i--)
			{
					...
					if (preg_match("/[A-Z]+/",$num)) { $num = ord($num) - 55; }

					elseif ($num == '-') { $num = 36; }				//	(TODO) use a switch
					elseif ($num == '.') { $num = 37; }				//	or matching array.
					elseif ($num == ' ') { $num = 38; }
					elseif ($num == '$') { $num = 39; }
					elseif ($num == '/') { $num = 40; }
					elseif ($num == '+') { $num = 41; }
	 ...
	 *	(DOCUMENTME)
	 *
	 *	(TODO) Find spec and add the link here
	 *	(TODO) Check for failures on GD calls
	 *
	 *	@param	string	$barnumber		Data to encode
	 *	@param	int	$scale			(DOCUMENTME)
	 *	@param	string	$file			Output image file? (CHECKME)
	 *	@param	bool	$checkdigit		(DOCUMENTME)
	 *	@return	void
	 */
	...
	function _c93Barcode($barnumber, $scale = 1, $file = "", $checkdigit = false)
	{
		...
		if (empty($file)) { header("Content-type: image/".$this->_format); }
		if ($scale < 1) { $scale = 2; }

		$total_y = (double)$scale * $this->_height + 10 * $scale;		//	(DOCUMENTME)

		//	commented out because $space is never intialized, strix 2012-04-10
		#if (!$space) {
		$space = array(
			'top' => 2 * $scale,
			'bottom' => 2 * $scale,
			'left' => 2 * $scale, 'right' => 2 * $scale
		);
		#}
		...
		//	previous code did not use switch, left in case of problems, strix 2012-04-10
		/*			
		if ($this->_format=="png")
		{
			if (!empty($file))
		...
		*/
		...
		switch (strtolower($this->_format)) {
			case "png":
				if (!empty($file)) { @imagepng($im, $file . "." . $this->_format); }
				else { @imagepng($im); }	//	header set above
				break;		//......................................................................

			case "gif":
				if (!empty($file)) { @imagegif($im, $file . "." . $this->_format); }
				else { @imagegif ($im); }	//	header set above
				break;		//......................................................................

			case "jpeg":	//	deliberate fallthrough
			case "jpg":
				if (!empty($file)) { @imagejpeg($im,$file.".".$this->_format); }
				else { @imagejpeg($im); }	//	header set above
				break;		//......................................................................
	...
	function _c39Encode($barnumber,$checkdigit = false)
	{
		...
		if (true == $checkdigit)
		{
			...
			for ($i = 0; $i < strlen($barnumber); $i++)
			{
				...
				if (preg_match("/[A-Z]+/", $num)) { $num= ord($num) - 55; }
				elseif ($num == '-') { $num = 36; }
				elseif ($num == '.') { $num = 37; }		//	(TODO) Use a switch or matching array
				elseif ($num == ' ') { $num = 38; }		//	for all these elseifs
				elseif ($num == '$') { $num = 39; }
				elseif ($num == '/') { $num = 40; }
				elseif ($num == '+') { $num = 41; }
				elseif ($num == '%') { $num = 42; }
		...
		for ($i = 0; $i < strlen($barnumber); $i++)
		{
			...
			{
				if ($tmp[$j]=='N' && $bar) { $mfcStr .= '1'; }
				else if ($tmp[$j] == 'N' && !$bar) { $mfcStr .= '0'; }
				else if ($tmp[$j] == 'W' && $bar) { $mfcStr .= $widebar; }
				else if ($tmp[$j] == 'W' && !$bar) { $mfcStr .= $widespc; }
	 ...
	 *	(DOCUMENTME)
	 *
	 *	(TODO) link to this barcode spec
	 *	Assumes $this->_format is one of 'png'|'jpeg'|'gif'
	 *
	 *	@param	string	$barnumber	Data to be encoded
	 *	@param	int	$scale		(DOCUMENTME)
	 *	@param	string	$file		Output image file
	 *	@param	bool	$checkdigit	(DOCUMENTME) See format spec
	 *	@return	void
	 */
	...
	function _c39Barcode($barnumber, $scale = 1, $file = '', $checkdigit = false)
	{
		...
		//	previous version without switch, remove when tested, strix 2012-04-10
		/*	
		if ($this->_format=="png")
		{
			if (!empty($file))
	 ...
	 *
	 *	@param	string	$barnumber	Data to be encoded
	 *	@param	bool	$useKeys	(DOCUMENTME)
	 *	@return	string
	 */

	function _c128Encode($barnumber, $useKeys)
	{
		//	(DOCUMENTME) source of this table, can it be procedurally generated
		$encTable = array(
		...
		if ('C' === $useKeys)
		{
			for ($i = 0; $i < strlen($barnumber); $i += 2)
			{
				...
				if (is_int($val)) { $sum += ($i + 1) * (int)($val); }
				elseif ($barnumber == chr(129)) { $sum += ($i + 1) * 100; }
				elseif ($barnumber == chr(130)) { $sum += ($i + 1) * 101; }
				$mfcStr .= $encTable[$val];
			}
		}
		else
		{
			for ($i = 0; $i < strlen($barnumber); $i++)
			{
				...
				if ($num>=32 && $num<=126) { $num = ord($barnumber[$i]) - 32; }
				elseif ($num == 128) { $num = 99; }
				elseif ($num == 129) { $num = 100; }
				elseif ($num == 130) { $num = 101; }
				elseif ($num < 32 && $useKeys === 'A') { $num = $num + 64; }
		...
		if ('A' === $useKeys) { $check = ($sum + 103) % 103; }
		if ('B' === $useKeys) { $check = ($sum + 104) % 103; }
		if ('C' === $useKeys) { $check = ($sum + 105) % 103; }
	...
	function _c128Barcode($barnumber, $scale = 1, $file = "")
	{
		...
		{
			$useKeys = 'C';
			if (strlen($barnumber) % 2 != 0) { $barnumber= '0' . $barnumber; }
		}
			
		for ($i = 0; $i < 32; $i++) { $chr = chr($i); }

		if (preg_match("/[" . $chr . "]+/", $barnumber)) { $useKeys='A'; }
		...
		#if (!$space)		//	uninitialized, so commented out
		#{
			$space = array(
				'top' => 2 * $scale,
				'bottom' => 2 * $scale,
		...
		//	(TODO) replace this with common output function
		if ($this->_format === "png")
		{
			if (!empty($file))
				@imagepng($im,$file.".".$this->_format);
	 ...
	 *	The data of the message, encoded from the table below, with a narrow inter-character space 
	...
	function _codaEncode($barnumber)
	{
		...
		$chrTable=array(
			"-" => "0001100",			"$" => "0011000",			":" => "1000101",
			"/" => "1010001",			"." => "1010100",			"+" => "0011111",
			"A" => "0011010",			"B" => "0001011",			"C" => "0101001",
			"D" => "0001110"
		);
		...
		for ($i = 0; $i < strlen($barnumber); $i++)
		{
			...
			for ($j = 0; $j < strlen($tmp); $j++)
			{
				if ($tmp[$j]=='0' && $bar) { $mfcStr.='1'; }
				else if ($tmp[$j] === '0' && !$bar) { $mfcStr .= '0'; }
				else if ($tmp[$j] === '1' && $bar) { $mfcStr .= $widebar; }
				else if ($tmp[$j] === '1' && !$bar) { $mfcStr .= $widespc; }
	...
	function _codaBarcode($barnumber,$scale=1,$file="")
	{
		...
		#if (!$space) {			//	commented out becase $space never initialized, strix 2012-04-10
			$space = array(
				'top' => 2 * $scale,
				'bottom' => 2 * $scale,
				'left' => 2 * $scale,
		...
		//	(TODO) use common output function for this
	...
	function _postEncode($barnumber)
	{
		...
		if ($sum % 10 != 0) { $check = (int)(10 - ($sum % 10)); }
	...
	{
		if (5 == strlen($barnumber) || 9 == strlen($barnumber) || 11 == strlen($barnumber))
		{
			//	(CHECKME) do nothing?
		}
		else
		...
		#if (!$space) {		// commented out because $space is never initialized, strix 2012-04-10
			$space = array(
				'top' => 2 * $scale,
				'bottom' => 2 * $scale,
				'left' => 2 * $scale,
	...
	function _i25Encode($barnumber)
	{
		...
		if ($len % 2 != 0)
		{
			...
			if ($len == strlen($barnumber) && substr($barnumber, -1) != '0') { $barnumber.='0'; }
		}
	...
	function _i25Barcode($barnumber, $scale = 1, $file = '')
	{
		...
		#if (!$space) {		//	never intialized, strix 2012-04-10
			$space = array(
				'top' => 2 * $scale,
				'bottom' => 2 * $scale,
				'left' => 2 * $scale, 'right' => 2 * $scale
	...
	/**
	 *	(DOCUMENTME)
	 *	@param	string	$upca	A UPCA data string (CHECKME)
	 *	@return	string
	 */
	...
	function ConvertUPCAtoUPCE($upca)
	{
		...
		if (strlen($upca) < 12 ) { $barnumber = str_pad($barnumber, 12, "0", STR_PAD_LEFT); }

		if ((substr($upca, 0, 1) != '0') && (substr($upca,0,1) != '1'))
		{
			$this->_error = 'Invalid Number System (only 0 & 1 are valid)';
			return false;		//	(TODO) return an empty string instead
		}
		else
		{
			if (
			...
			else
			{
				...
				return false;		//	(TODO) return empty string as error value
			}
		}
		return $upce;
	}
	 ...
	 *	(DOCUMENTME)
	 *
	 *	(TODO) link to the UPCE spec
	 *
	 *	@param	string	$barnumber	Data to be encoded
	 *	@param	int		$encbit		0 or 1 (DOCUMENTME)
	 *	@param	int		$checkdigit	(DOCUMENTME)
	 *	@return	string
	 */
	...
	function _upceEncode($barnumber, $encbit, $checkdigit)
	{
		...
		if ($encbit == 0) { $encTable = $encTable0; }
		elseif ($encbit == 1) { $encTable=$encTable1; }
		else
		{
		...
		for ($i = 0; $i < strlen($barnumber); $i++)
		{
			$num = (int)$barnumber{$i};		//	curly braces?
			$even = (substr($encTable[$checkdigit], $i, 1) === 'E');

			if (!$even) { $mfcStr .= $leftOdd[$num]; }
			else { $mfcStr .= $leftEven[$num]; }
		}
	...
	function _upceBarcode($barnumber,$scale=1,$file="")
	{
		...
		//	(TODO) use common output function to return image
		if ($this->_format === "png")
		{
			if (!empty($file)) { @imagepng($im,$file.".".$this->_format); }
			else { @imagepng($im); }
	 ...
	 *	(DOCUMENTME)
	 *	@param	string	$barnumber	Data to be encoded
	 *	@param	string	$number		(DOCUMENTME)
	 *	@return	string
	 */
	...
	function _checkDigit($barnumber, $number)
	{
		...
		for ($i = 0; $i < strlen($barnumber); $i++) 
		{
			if ($i % 2 == 0 ) { $csumTotal = $csumTotal + (3 * intval($barnumber{$i})); }
			else { $csumTotal = $csumTotal + intval($barnumber{$i}); }
		}

		//	Calculate the checksum digit
		if ($csumTotal % 10 == 0) { $checksumDigit = ''; }
		else { $checksumDigit = 10 - ($csumTotal % 10); }
	...
	function _ean8Encode($barnumber)
	{
		...
		{
			$num = (int)$barnumber{$i};
			if ($i < 4) { $mfcStr .= $leftOdd[$num]; }
			elseif ($i >= 4) { $prodStr .= $rightAll[$num]; }
		}
	 ...
	 *	@param	string	$barnumber	Data to be encoded
	 *	@param	int	$scale		(DOCUMENTME)
	 *	@param	string	$file		Output image document
	 */
	...
	function _ean8Barcode($barnumber, $scale = 1, $file = '')
	{
		...
		//	(TODO) use global output function for this
	...
	function _ean13CheckDigit($barnumber)
	{
		...
		//	Commented out when found, strix 2012-04-10
		#if (strlen($barnumber) == 13)
		...
		for ($i = 0; $i < strlen($barnumber); $i++) 
		{
			if ($i % 2 == 0) { $csumTotal = $csumTotal + intval($barnumber{$i}); }
			else { $csumTotal = $csumTotal + (3 * intval($barnumber{$i})); }
		}
	...
	function _eanBarcode($barnumber, $scale = 1, $file = '')
	{
		...
		if ($this->_encode === 'UPC-A') { $str = substr($barnumber, 1, 1); }
		else { $str = substr($barnumber, 0, 1); }

		/* add text ----------------------------------------------------------------------------- */
		@imagettftext($im, $scale * 6, 0, $space['left'], (int)$height, $bar_color, $this->_font, $str);

		if ($this->_encode === 'UPC-A') { $str = substr($barnumber, 2, 5); }
		else { $str = substr($barnumber, 1, 6); }

		$x = $space['left'] + $scale * strlen($barnumber) + $scale * 6;
		@imagettftext($im, $scale * 6, 0, $x, (int)$height2, $bar_color, $this->_font, $str);
			
		if ($this->_encode === 'UPC-A') { $str = substr($barnumber, 7, 5); }
		else { $str = substr($barnumber, 7, 6); }

		$x = (int)$space['left'] + $scale * strlen($bars) / 1.65 + $scale * 6;
		...
		if ($this->_encode === 'UPC-A')
		{
			...
			$x = $total_x - $space['left'] - $scale * 6;
			@imagettftext($im, ($scale * 6), 0, $x, (int)$height, $bar_color, $this->_font , $str);
		}
