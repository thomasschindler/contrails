<?
	...
	require_once(__DIR__ . '/oos.cnf.inc.php');
...
*/
	...
	class imagemagick {
		...
		var $temp_dir		= '/usr/local/djinny/projects/inc/tmp'; // httpd must be able to write there
		...
		var $verbose        = false;
			...
			*/
			...
			{
				$this->imagemagickdir 	= CONF::bin_dir();
				$this->temp_dir			= CONF::tmp_dir();

				$this->temp_file = time().preg_replace("/[^a-zA-Z0-9_.]/", '_', $filedata['name']);
				
				if(!@rename($filedata['tmp_name'], $this->temp_dir.'/tmp'.$this->count.'_'.$this->temp_file))
					die("Imagemagick: Upload failed for:".$this->temp_file);

				$path = $this->temp_dir.'/tmp'.$this->count.'_'.$this->temp_file;
								
				chmod($path,0777);
				
				$this->file_history[] = $path;
				$this->GetSize();
			}
			...
			*/
			...
			function GetSize() {
				...
				for($i=0;$i<$num;$i++)
					if(preg_match('/Geometry/', $this->image_data[$i])) {
			...
			*/
			...
			function Resize($x_size = 9999999, $y_size = 9999999, $how='keep_aspect') {
			...
			*/
			...
			function Save($prefix='',$only=false) {
				if($this->verbose == TRUE) {
					echo "Save:\n";
				}
				
				if($only == false){
					$target_file = $this->temp_file;
				}
				else{
					$target_file = "";
				}
				
				if(!@copy($this->temp_dir.'/tmp'.$this->count.'_'.$this->temp_file, $this->targetdir.'/'.$prefix.$target_file)) {
					...
					chmod($this->targetdir.'/'.$prefix.$target_file,0755);
				}
