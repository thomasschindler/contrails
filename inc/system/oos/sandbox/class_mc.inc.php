<?php
	...
	require_once(__DIR__ . '/class_opc.inc.php');
...
class MC {
	...
	function call_action($action, $params = null, $is_static = false) 
	{
		...
		$mod = MC::get_mod_instance($modul);				// mapping of mod-name in class	
	...
	function call($mod, $event = null, $params = null, $background = 0, $spoolpath = null)
	{
		if ($background === 1)
		{           
			$OPC = &OPC::singleton();
		...
		return $this->call_action(array('mod' => $mod, 'event' => $event), $params, false);
	}
	...
	public function get_mod_instance($modul, $is_static = false, $constructor = true)
	{
		...
		require_once($inc_dir.'system/class_modAction.inc.php');
	...
	function get_access_rights($pid = -1, $uid = -1) 
	{
		...
		/*
			static $ar_read = false;

			if ($ar_read[$pid] === true)
			{
				MC::debug($this->access_rights);
	...
	*/
	...
	public function create($class_name, $a = null, $b = null) {
		...
		require_once(CONF::inc_dir() . $package . '/' . $class_file);
	 ...
	 *	@param	string	$path	Location of log files, optional
	 *	@return	string	Location of the written log file
	 */
	function log($log, $name = '', $path = '')
	{
		if ('' === $name) { $file = "mc_default.log"; }
		else { $file = $name; }

		if ('' === $path) { $file = CONF::inc_dir() . "tmp/" . $file; }
		else { $file = $path . "/" . $file; }
	...
	function call_listeners($mod, $event, $position = 0, $add_params = null)
	{
		...
		if (!is_array(@$this->listeners)) { return true; }
