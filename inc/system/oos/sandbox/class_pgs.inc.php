...
class pgs {
  ...
  function mostra ($args=array()) {
    ...
    $_form = array();
    if ($args['open_form'])
      $_form[] = '<form name="pagseguro" id="pagseguro" action="https://pagseguro.uol.com.br/security/webpagamentos/webpagto.aspx" method="post">';
	...
	function request_get()
	{
	    ...
	    foreach ($this->_itens as $item) 
		{
			...
			$_form[] = str_replace ('.', '', sprintf ('%s=%.2f', "item_valor$sufixo", $item['valor']));
			unset($item['valor']);
		...
		return 'https://pagseguro.uol.com.br/security/webpagamentos/webpagto.aspx?'.implode("&",$_form);
	}
	...
	{                          
		...
		ob_clean();
		header('Location: https://pagseguro.uol.com.br/security/webpagamentos/webpagto.aspx?'.implode("&",$_form));
		die;
