...
{	
	...
	(
        ...
        "mp3" => array("audio/mpeg3","audio/x-mpeg-3","audio/mpeg","video/mpeg","video/x-mpeg"),
	...
	function mime()
	{
		// create the reverse version of the type2mime array
		foreach($this->get_mime as $key => $val)
		{
			foreach($val as $mime)
			{
				$this->get_type[$mime][] = $key;
			}
		}
	}
	
	function get_mime($type,$first=true)
	{
		if($this->get_mime[$type])
		{
			if($first)
			{
				return $this->get_mime[$type][0];
			}
			else
			{
				return $this->get_mime[$type];
			}
		}
		return false;
	}

	function get_type($mime,$first=true)
	{
		if($this->get_type[$mime])
		{
			if($first)
			{
				return $this->get_type[$mime][0];
			}
			else
			{
				return $this->get_type[$mime];
			}
		}
		return false;
	}
