...
#		exec('find '.$this->burc_dir.'/ -amin +120 -exec rm {} ";" ');
	...
	*/
	function parse($url)
	{
		...
		{
			if(preg_match("/.html/",$part))
			{
				$test = explode(".html",$part);
				$select = "SELECT * FROM sys_burc WHERE burc ='".$test[0]."'";
	...
	*/
	function lnk($burc,$anchor=null,$filetype="html",$permanent=0)
	{
		...
		return $this->set($burc,$permanent).".".$filetype.$sess_info['current'];
	}
	/**
	...
	*/
	function set($burc,$permanent=0)
	{
		...
		'".$burc_."',
		".UTIL::get_post('pid').",
		'".serialize($this->rectify_array($burc))."',
		".time().",
		".$permanent."
		)";		
