<?php
	...
	require_once(__DIR__ . '/class_sess.inc.php');
	require_once(__DIR__ . '/class_mc.inc.php');
...
class OPC 
{
	...
	*/
	...
	{
		//echo "OPC calling $modul_name :: $method_name :: $vid <br/>\n";
		return $this->call_view($vid,$modul_name,$method_name,(!$vid ? 0 : 1));
	}
	...
	function call_view($view_name, $modul_name, $method_name = '', $force_view_name = false)
	{
		#MC::debug($this->set_view,$view_name);

		$this->lang_page_start($modul_name);

		//	we need to check whether this module is supposed to be called remotely
		//	if so, and there wasn't an explicit event on this mod, we need to call the default 
		//	values - for the default view

		if(!$this->remote_connection)
		{
			$cs_server = CONF::get('content_syndication_server');
			$this->remote_connection = &remote_connection::singleton($cs_server);
		}
			
		$this->remote_connection->call_view($view_name,$modul_name,$method_name);

		$inc_dir = CONF::inc_dir();
			
		//	[en] Mapping of mod name to class
		//	[en] Now 'on the fast', later better an array (?) 'Mod_name' => 'class_name'
		//	[de] mapping von mod-name auf klasse
		//	[de] jetzt 'auf die schnelle', spaeter besser ein array (?) 'mod_name' => 'class_name'

		$class_dir  = $inc_dir . 'mods/' . $modul_name . '/class_' . $modul_name . 'View.inc.php';

		$class_name = $modul_name . '_view';
						
		if (!file_exists($class_dir)) 
		{
			echo 'ERROR: new view found [' . $modul_name . ']';
			return;
		}
			
		//	[en] include the module view class
		//	[en] PHPLint throws a tantrum at this point, may not be testable with this tool
		//	[de] view-klasse des moduls einbinden
		...
		if (method_exists($mod, '_constructor')) { 	// _TEMP
			$mod->_constructor();
		}

		/**
			stack needs to be incremented at the current level.
			then we need to check for values in 
			
			this patch corrects the force view name handling which was not working up to now
			be very careful when adding this patch to running systems because all your content 
			might vanish!!!!!!!!!!!!			
		*/
		
		if(CONF::patch('270807_ts_opc_call_view'))
		{
			//awrkward, clarify, strix 2012-02-20
			if(false == $force_view_name) { $this->__stack[sizeof($this->__stack) - 1]++; }
		}
		else
		{
			$this->__stack[sizeof($this->__stack) - 1]++;
		}
		
		if (true == $force_view_name) {
			$current_view = $view_name;
		}			
		else
		{
			$this->position_info = $this->get_position_info($view_name,$modul_name);
			$current_view = isset($this->position_info["vid"]) ? $this->position_info["vid"] : $view_name;
		}

		// set the current vid to a global vid -> check to see if that works!!! ;-}
		$this->__current_vid = $current_view;
		$this->__current_mod = $modul_name;

		//	[en] method name can have been set by $this->set_view() (CHECKME)
		//	[de] methoden-name kann ueber set_view() gesetzt worden sein
		if (@isset($this->set_view[$modul_name][$current_view])) {
			$method_name = $this->set_view[$modul_name][$current_view];
		}
						
		$this->lang_page_end();
						
		// set the display row and column in the module
		return $mod->main($current_view, $method_name);
	}
	...
	{
		if (is_array($this->_vars['__remote'][$this->__current_vid]))
		{
			#MC::debug($this->_vars[$this->__current_mod]);
			#$this->_vars[$this->__current_mod] = array_merge(
			#	$this->_vars[$this->__current_mod],
			#	$this->_vars['__remote'][$this->__current_vid]
			#);
			#unset($this->_vars['__remote'][$this->__current_vid]);
			#unset($this->set_view);
			$this->_vars[$this->__current_mod] = $this->_vars['__remote'][$this->__current_vid];
		}
		return;
	}
