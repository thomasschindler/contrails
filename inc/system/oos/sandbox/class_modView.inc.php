...
class modView {
	...
	*/
	...
	function generate_view($tpl, $add_path = false)
	{
		...
		if ($this->MC->access("page","ar_mod")) {

			$args = array(
				'data[pid]' => $this->pid,
				'data[mid]' => $this->MC->get_modul_id($this->mod_name),
				'mod' => 'page',
				'event' => 'pa_ar',
				'pid' => $this->pid,
			);

			$url = $this->OPC->lnk($args);

			$headline = ''
			 . '<div style="position:absolute;left:0px;">'
			 . '<a href="' . $url . '" target="_blank" onClick="return popup(\'' . $url . '\',900);">'
			 . $this->OPC->show_icon('rights_red') . '</a>'
			 . '</div> '
			 . (string)$this->get_var("headline");

			$this->set_var("headline", $headline);
		}
	...
	public function _constructor() {
		...
		*/
		...
		if (!is_dir($this->tpl_dir))
		{
			$this->tpl_dir = CONF::inc_dir() . 'mods/' . $this->mod_name . '/template/oos/';
		}			
			
		$this->MC->__current_mod = $this->mod_name;
			
	  	// global rsc (resource?) directory ---
		$this->rsc_dir  = CONF::inc_dir() . 'mods/' . $this->mod_name . '/rsc/';
